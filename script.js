...
class WebGLFrame {
    ...
    constructor(){
        ...
        this.tmpMatrix = MAT.identity(MAT.create());
        this.mvpMatrix = MAT.identity(MAT.create());
        this.invMatrix = MAT.identity(MAT.create());
    ...
    setup(){
        ...
        this.canvas.addEventListener('mousemove', (e) => {
            this.mousX = e.offsetX / this.canvas.width;
            this.mousY = e.offsetY / this.canvas.height;
        }, false);

        const VERTEX_COUNT = 100;
        const SPHERE_LAYER = 300;
        const RADIUS = 2.0;
        this.vertex_position = [];
        this.vertex_color = [];
        let h = 0;
        let vertex_radius = RADIUS - (h / SPHERE_LAYER);
        let tmp_radius = null;
        let tmp_x = null;
        let tmp_y = null;
        let tmp_z = null;
        ...
        do{
            tmp_x = this.getRandom();
            tmp_y = this.getRandom();
            tmp_z = this.getRandom();
            tmp_radius = RADIUS - (h / SPHERE_LAYER) * 10;
            if(h >= 10){ vertex_radius = tmp_radius; }
            ...
            for(let i = 0; i < VERTEX_COUNT; ++i){
              ...
              for(let j = 0; j < VERTEX_COUNT; ++j){
                ...
                this.vertex_position.push(x * vertex_radius * radius + tmp_x,
                                          y * vertex_radius + tmp_y,
                                          z * vertex_radius * radius + tmp_z);
                this.vertex_color.push(0.0, 0.0, 0.0);
              }
            }
            ++h;
        }while(vertex_radius > 0);
    ...
    render(time){
        ...
        this.vMatrix = MAT.lookAt(cameraPosition, centerPoint, cameraUpDirection, this.vMatrix);
        this.pMatrix = MAT.perspective(fovy, aspect, near, far, this.pMatrix);
        this.vpMatrix = MAT.multiply(this.pMatrix, this.vMatrix, this.vpMatrix);

        var rad = (this.nowTime % 360) * Math.PI / 180;
        this.mMatrix = MAT.identity(this.mMatrix);
        this.mMatrix = MAT.rotate(this.mMatrix, rad * 30, [0, 1, 1], this.mMatrix);
        this.mvpMatrix = MAT.multiply(this.vpMatrix, this.mMatrix, this.mvpMatrix);
        ...
        gl.drawArrays(gl.LINE_LOOP, 0, this.vertex_position.length / 3);
    }
    ...
    getRandom(){
        return Math.random() * (10.0 - (-10.0)) + (-10.0);
    }
}
